<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Estado amigos.</title>
    <?php include('1_css.php')?>
</head>
<body class="cream_ours_color_dark">
    <!--Menu de página de inicio-->
    <nav class="cooper_ours_color_dark z-depth-3">
        <div class="nav-wrapper">
        <a href="#" class="brand-logo hide-on-small-only">&nbsp;&nbsp;&nbsp;Estado amigos.</a>
        <a href="b_opciones_intercambios.php" data-target="slide-out" class="sidenav-trigger"><i class="material-icons">arrow_back</i></a>
        <p class="titleMobile flow-text hide-on-med-and-up">Estado amigos.</p>
        <ul class="right hide-on-med-and-down">
            <li><a href="b_opciones_intercambios.php">Volver al inicio</a></li>
        </ul>
        </div>
    </nav>

    <div class="title_all">
        <p class="black-text">Estado de amigos.</p>
    </div>

    <section class="containerGeneral cream_ours_color_dark">
        <div class="containerCardsContador row">
            <div class="col s12 m12 l6 cardPadreContainer">
                    <div class="cardContador cultured_ours_color z-depth-2 waves-effect">
                        <div class="cardContadorTitle flow-text">
                            Amigo X
                        </div>
                        <div class="cardContadorImage">
                            <span class="new badge badgeDiseno" data-badge-caption="">Listo</span>
                        </div>
                    </div>
            </div>
            <div class="col s12 m12 l6 cardPadreContainer">
                    <div class="cardContador cultured_ours_color z-depth-2 waves-effect">
                        <div class="cardContadorTitle flow-text">
                            Amigo X
                        </div>
                        <div class="cardContadorImage">
                            <span class="new badge badgeDiseno" data-badge-caption="">No listo</span>
                        </div>
                    </div>
            </div>
            <div class="col s12 m12 l6 cardPadreContainer">
                    <div class="cardContador cultured_ours_color z-depth-2 waves-effect">
                        <div class="cardContadorTitle flow-text">
                            Amigo X
                        </div>
                        <div class="cardContadorImage">
                            <span class="new badge badgeDiseno" data-badge-caption="">Listo</span>
                        </div>
                    </div>
            </div>
        </div>
    </section>


</body>
<?php include '1_js.php'; ?>
</html>